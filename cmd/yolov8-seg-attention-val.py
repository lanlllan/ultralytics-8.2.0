...
def print_comparison(results, split, baseline_key="baseline"):
    ...
    sep = "=" * 130
    print(f"\n{sep}")
    print(f"  注意力机制对比评估  |  data={DATA_YAML}  split={split}  imgsz={IMGSZ}")
    print(sep)

    header = (
        f"{'实验':>12} | {'注意力':>8} {'位置':>14} | {'参数量':>10} {'增量':>8} |"
        f" {'M_mAP50':>8} {'M_mAP95':>8} {'提升':>7} |"
    ...
    for exp_name in sorted_keys:
        ...
        print(
            ...
            f" {r['mask_map50']:>8.4f} {r['mask_map']:>8.4f} {delta_map_str:>7} |"
    ...
    if best_name != baseline_key and baseline:
        ...
        print(f"\n  注意: 所有注意力方案均未超过基线 mask_mAP50-95")
